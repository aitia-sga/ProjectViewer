...
export function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(
		...
		// vscode.commands.registerCommand('projectViewer.addToProject', async (fileUris: vscode.Uri[] | vscode.Uri) => {		
		// 	if (!fileUris) { 
		// 		vscode.window.showErrorMessage('fileUri is empty!'); 
		// 		return; 
		// 	}
		
		// 	// Make sure we're working with an array of Uris
		...
		// 		fs.stat(fileUri.fsPath, (err: any, stats: any) => {
		// 			if(err) {
		// 				vscode.window.showErrorMessage(`Cannot retrieve file information: ${err.message}`);
		// 			} else if(stats.isDirectory()) {
		// 				fileType = "physicalDirectory";
		// 			}
		// 		});
		...
		// 			myProjects.addFileToProject(selectedItem, fileUri, fileType, description);
		// 		} catch (err: any) {
		// 			vscode.window.showErrorMessage(`An error occurred while adding file ${fileUri.fsPath}: ${err.message}`);
		// 		}
		// 	}
		...
		// vscode.commands.registerCommand('projectViewer.addToProject',  (...commandArgs) => {

		// 	console.log("djhsfhnsdikfu");
		// }),
